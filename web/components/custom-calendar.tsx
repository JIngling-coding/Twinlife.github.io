...
function CustomCalendar({ className, classNames, showOutsideDays = true, ...props }: CalendarProps) {
  ...
  const [editingYear, setEditingYear] = React.useState("")
  const yearInputRef = React.useRef<HTMLInputElement>(null)

  const handleYearDoubleClick = () => {
    setIsEditingYear(true)
    setEditingYear(props.selected ? props.selected.getFullYear().toString() : new Date().getFullYear().toString())
  }
        ...
        Caption: ({ displayMonth }) => (
          <div className="flex items-center justify-center relative">
            <div className="flex items-center gap-1">
              ...
              <span>{displayMonth.toLocaleString("default", { month: "long" })}</span>
            </div>
          </div>
        ),
      }}
